...
class Solver {
  ...
  css(filters) {
    return `filter: ${this.raw(filters)};`;
  }
}

// Fungsi utama: hex -> CSS filter
function hexToCssFilter(hex) {
  const rgb = hexToRgb(hex);
  if (!rgb) throw new Error("Invalid hex color");
  const color = new Color(rgb[0], rgb[1], rgb[2]);
  const solver = new Solver(color);
  return solver.solve(); // { filter, filterRaw, loss, values }
}

// Contoh pakai:
// const result = hexToCssFilter("#3498db");
// console.log(result.filter);    // "filter: brightness(0) saturate(100%) invert(...) ..."
// console.log(result.filterRaw); // tanpa "filter:" prefix
// console.log(result.loss);      // tingkat akurasi (semakin kecil semakin akurat)
